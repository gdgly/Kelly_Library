...
void Menu_StartPrevMenu()
{
	if(MenuSelect->PrevMenu)
	{
		MenuSelect = MenuSelect->PrevMenu;
		if(MenuSelect->InitFunction)	MenuSelect->InitFunction();
	}
}

void Menu_DoMainFunction()
{
	if (MenuSelect->MainFunction) MenuSelect->MainFunction();
}
...
//void Menu_InitMenuFunction(MENU_T * target, ...)
//{
//
//}
