...
****************************************************************************//*!
*
* @brief  FreeMASTER Driver main API header file, HC(S)08 platform
*
...
typedef unsigned char * FMSTR_ADDR;             /* CPU address type */
typedef unsigned short FMSTR_SIZE;              /* general size type (at least 16 bits) */
typedef unsigned short FMSTR_SIZE32;            /* general size type (at least size of address (can be 16bits on S08)) */
...
typedef unsigned char  FMSTR_PIPE_PORT;         /* pipe port identifier (unsigned, 7 bits used) */
...
/* CodeWarrior compiler: make sure the ISRs are in non-banked memory */
#if defined(__MWERKS__) && defined(__BANKED__)
#pragma CODE_SEG __NEAR_SEG NON_BANKED
#endif
void FMSTR_Isr(void);           /* SCI or MSCAN interrupt handler for LONG_INTR and SHORT_INTR modes */
